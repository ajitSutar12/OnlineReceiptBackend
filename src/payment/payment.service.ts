...
export class PaymentService {
    ...
    async invoiceList() {
        let connection1 = await oracledb.getConnection({ user: "BWAYSCOMM", password: "BWAYSCOMM", connectString: "(DESCRIPTION =(ADDRESS = (PROTOCOL = TCP)(HOST = ERP3.erpcompserv.local)(PORT = 1521))(CONNECT_DATA =(SID= ORCL)))" });
        let query = await connection1.execute(`SELECT CAST(TRNACCTMATH.TRAN_NO AS VARCHAR(18)) AS REFTRAN_NO, FORMAT_DATE(TRNACCTMATH.TRAN_DATE) PRN_TRANDATE, TRNACCTMATH.TRAN_DATE, TRAN_AMT, TRNACCTMATH.TRANFORMAT_NO,        NVL(TRNACCTMATH.TRANFORMAT_NO,TRNACCTMATH.FIN_YEAR || ' - ' || CAST(CAST(SUBSTR(CAST(TRNACCTMATH.TRAN_NO AS VARCHAR(18)),12,7) AS NUMBER) AS VARCHAR(7))) || ' - ' || (CASE WHEN MSTACCTGLSUB.SUB_GLCODE=21 THEN MSTACCTGLSUB.SUBGL_NAME ELSE PAID_TO END) PAID_TO,        TRNACCTMATH.SUB_GLACNO,        (CASE WHEN MSTACCTGLSUB.SUB_GLCODE=21 THEN MSTACCTGLSUB.SUBGL_NAME ELSE PAID_TO END) PARTY_NAME, MSTACCTGLSUB.GSTIN_NO FROM    (   SELECT  FIN_YEAR,TRAN_NO,TRANFORMAT_NO,SUB_GLACNO,PAID_TO, TRAN_DATE, TRAN_AMT            FROM    BWAYSFAS.TRNACCTMATH             WHERE   TRNACCTMATH.TRAN_TYPE=115                      AND TRNACCTMATH.STATUS_CODE=0        )TRNACCTMATH        INNER JOIN BWAYSFAS.MSTACCTGLSUB ON MSTACCTGLSUB.SUB_GLACNO=TRNACCTMATH.SUB_GLACNO WHERE   NOT EXISTS (	SELECT  trnacctcommrcpth.REF_TRANNO    FROM    trnacctcommrcpth                        WHERE 	trnacctcommrcpth.REF_TRANNO	 = TRNACCTMATH.TRAN_NO   AND trnacctcommrcpth.STATUS_CODE=0)`)
        return this.jsonConverter(query);
